...
def add_new_card_for_student(session: db.orm.session.Session):
    ...
    student: Student = session.query(Student).filter(Student.student_id == 2).first()
    if student:
        card = LibraryCard(card_id=4, student_id=2)
        student.library_card = card
        session.commit()


def connect():
    engine = db.create_engine('sqlite:///example_one_to_one.db', echo = True) #set echo = Tru to see sql log
    return engine
